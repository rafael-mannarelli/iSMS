function out = scatplot(x,y,method,radius,N,n,po,ms,colmap,ax)
...
switch method %Calculate Data Density
    case 'sq'  %---- Using squares ----
        myprogressbar('Calculating S-E density plot')
        for k=1:Ld
    ...
    case 'vo'  %----- Using voronoi cells ------
        ...
        myprogressbar('Calculating S-E density plot')
        n = length(c);
        t = 1000; % Progressbar update factor
        for k=1:n 
...
%~~~~~~~~~~ Graf Scatter Plot ~~~~~~~~~~~
function varargout = gsp(x,y,c,ms,colmap,ax)
axes(ax)
