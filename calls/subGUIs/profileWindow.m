function varargout = profileWindow(varargin) %% Initializes the GUI
% PROFILEWINDOW - GUI window associated with iSMS for analysing laser
% profiles
%
%  profileWindow.m cannot be by called by itself as it relies on handles
%  sent by the sms.m main figure window upon opening.
%
%  The profileWindow GUI is programmed using GUIDE. The callbacks of
%  the GUI is found in the profileWindow.m file and is divided into
%  sections of:
...
% Last Modified by GUIDE v2.5 13-Jan-2014 16:38:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @profileWindow_OpeningFcn, ...
    'gui_OutputFcn',  @profileWindow_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function profileWindow_OpeningFcn(hObject, eventdata, handles, varargin) %% Executes just before Profile Editor is made visible
...
% Choose default command line output for profileWindow
...
function varargout = profileWindow_OutputFcn(hObject, ~, handles) %% Outputs from this function are returned to the command line
% Show GUI
...
if get(handles.greenRadiobutton,'Value') && (~isempty(handles.green))
    choice = get(handles.greenListbox,'Value');
    image = double(handles.green(choice).image);
    roi = handles.green(choice).ROI;
    spot = 1;
elseif get(handles.redRadiobutton,'Value') && (~isempty(handles.red))
    choice = get(handles.redListbox,'Value');
    image = double(handles.red(choice).image);
    roi = handles.red(choice).ROI;
    spot = 2;
else return
end
...
pars = fit2Dgauss(handles, image, roi, spot);
...
% Store result
if get(handles.greenRadiobutton,'Value') && (~isempty(handles.green))
    handles.green(end+1).name = sprintf('Fit: %s',handles.green(choice).name);
    handles.green(end).image = fit;
    handles.green(end).raw = fit;
    handles.green(end).pars = pars;
    handles.green(end).ROI = handles.green(choice).ROI;
    handles.green(end).existing = 0;
    handles.green(end).measured = 0;
    ...
    set(handles.greenListbox,'Value',length(handles.green));
elseif get(handles.redRadiobutton,'Value') && (~isempty(handles.red))
    handles.red(end+1).name = sprintf('Fit: %s',handles.red(choice).name);
    handles.red(end).image = fit;
    handles.red(end).raw = fit;
    handles.red(end).pars = pars;
    handles.red(end).ROI = handles.red(choice).ROI;
    handles.red(end).existing = 0;
    handles.red(end).measured = 0;
...
% Green profiles:
new = 0; % Determines whether profile already exists in the sms window
for i = 1:length(handles.green)
    
    % Make normalized Gaussian profile image with zero background
    pars = handles.green(i).pars;
%     pars(1) = 1/(2*pi*pars(3)*pars(5)); % normalize area
%     pars(7) = 0; % Set background to zero
    image = make2Dgauss([size(handles.green(i).image,1) size(handles.green(i).image,2)], pars);
    
    %     image = handles.green(i).image;
    
    if handles.green(i).existing~=0 % If profile i is already existing in the sms window
        for j = 1:length(mainhandles.data)
            file = [];
            if strcmp(mainhandles.data(j).name,handles.green(i).name)
                file = j;
                break
            end
        end
        if isempty(file) % If make new file
            new = 1;
        else % Update data file info
            mainhandles.data(file).avgimage = image;
            mainhandles.data(file).avgDimage = image;
            mainhandles.data(file).avgAimage = image;
            mainhandles.data(file).imageData = cat(3,image,image);
            mainhandles.data(file).avgimageFrames = [1 1]; % First and last frame in the avg. image generation [first last]
            mainhandles.data(file).spot = 1;
            if length(mainhandles.data(file).excorder)>=2
                mainhandles.data(file).excorder = mainhandles.data(file).excorder(1:2);
            else
                mainhandles.data(file).excorder = 'D';
            end
        end
    else % If make new file
        new = 1;
    end
    
    % If not already existing, make new data file with green profile i
    if new
        % If profile size does not match selected movie size see if it
        % matches another file, and if not then don't export the profile
        filechoice = get(mainhandles.FilesListbox,'Value');
        if ~isequal(size(mainhandles.data(filechoice).avgimage), size(image))
            filechoice = [];
            for j = 1:length(mainhandles.data)
                if isequal(size(mainhandles.data(j).avgimage), size(image))
                    filechoice = j;
                end
            end
            if isempty(filechoice)
                mymsgbox(sprintf(...
                    'The size of profile %s does not match any of the sizes of the currently loaded data movies. This profile is therefore not exported',...
                    handles.green(i).name));
                break
            end
        end
        
        % Create a new data set with field values equal to currently
        % selected movie file
        fnames = fieldnames(mainhandles.data(filechoice));
        for j = 1:length(fnames)
            if j == 1
                mainhandles.data(end+1) = setfield(mainhandles.data(filechoice),fnames{j},mainhandles.data(filechoice).(fnames{j}));
            else
                mainhandles.data(end) = setfield(mainhandles.data(filechoice),fnames{j},mainhandles.data(filechoice).(fnames{j}));
            end
        end
        
        % Insert profile information into handles structure
        mainhandles.data(end).name = handles.green(i).name;
        mainhandles.data(end).spot = 1;
        mainhandles.data(end).spotMeasured = handles.green(i).measured;
        mainhandles.data(end).avgimage = image;
        mainhandles.data(end).avgDimage = image;
        mainhandles.data(end).avgAimage = image;
        mainhandles.data(end).Droi = handles.green(i).ROI;
        mainhandles.data(end).avgimageFrames = [1 1]; % First and last frame in the avg. image generation [first last]
        if length(mainhandles.data(filechoice).excorder)>=2
            mainhandles.data(end).excorder = mainhandles.data(filechoice).excorder(1:2);
        else
            mainhandles.data(end).excorder = 'D';
        end
        
        % Delete info coming from the template file
        mainhandles.data(end).imageData = cat(3,image,image);
        mainhandles.data(end).DD_ROImovie = [];
        mainhandles.data(end).AD_ROImovie = [];
        mainhandles.data(end).DA_ROImovie = [];
        mainhandles.data(end).AA_ROImovie = [];
        mainhandles.data(end).FRETpairs = [];
        mainhandles.data(end).Dpeaks = [];
...
for i = 1:length(handles.red)
    % Make normalized Gaussian profile image with zero background
    pars = handles.red(i).pars;
%     pars(1) = 1/(2*pi*pars(3)*pars(5)); % normalize area
%     pars(7) = 0; % Set background to zero
    image = make2Dgauss([size(handles.red(i).image,1) size(handles.red(i).image,2)], pars);
    %     image = handles.red(i).image;
    
    if handles.red(i).existing~=0 % If profile already exists as a data file in sms
        % Determine which file
        for j = 1:length(mainhandles.data)
            file = [];
            if strcmp(mainhandles.data(j).name,handles.red(i).name)
                file = j;
                break
            end
        end
    ...
    % If not already existing, make new data file with red profile i
    if new
        % If profile size does not match selected movie size see if it
        % matches another file, and if not then don't export the profile
        filechoice = get(mainhandles.FilesListbox,'Value');
        ...
        % Insert profile information into handles structure
        mainhandles.data(end).name = handles.red(i).name;
        mainhandles.data(end).spot = 2;
        mainhandles.data(end).spotMeasured = handles.red(i).measured;
        mainhandles.data(end).avgimage = image;
        mainhandles.data(end).avgDimage = image;
        mainhandles.data(end).avgAimage = image;
