...
%         % Alpha map
%         alphaMap = getappdata(0,'alphaMap');

        % Convert to 3xWxH format
        data = permute(data, [3, 2, 1]);
%         alphaMap = uint8(255*permute(alphaMap, [3, 2, 1]));
%         whos data
%         whos alphaMap
%         
        % Append Alpha data (not used)
%         if ~isempty(alphaMap) && isequal(size(data,2),size(alphaMap,2)) && isequal(size(data,3),size(alphaMap,3))
%             k=1
%             data = cat(1, data, uint8(255*alphaMap));
%             whos data
%         else
            data = cat(1, data, 25*ones(1, wd, ht, 'uint8'));
%         end
