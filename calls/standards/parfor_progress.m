...
if N > 0
    f = fopen(fullfile(tempdir,'parfor_progress.txt'), 'w');
    if f<0
...
elseif N == 0
    delete(fullfile(tempdir,'parfor_progress.txt'));
    percent = 100;
    ...
    if ~exist(fullfile(tempdir,'parfor_progress.txt'), 'file')
        error('parfor_progress.txt not found. Run PARFOR_PROGRESS(N) before PARFOR_PROGRESS to initialize parfor_progress.txt.');
    end
    
    f = fopen(fullfile(tempdir,'parfor_progress.txt'), 'a');
    fprintf(f, '1\n');
    fclose(f);
    
    f = fopen(fullfile(tempdir,'parfor_progress.txt'), 'r');
    progress = fscanf(f, '%d');
    fclose(f);
    ...
    if nargout == 0
        ...
        message = [repmat(char(8), 1, (w+9)), char(10), perc, '[', repmat('=', 1, round(percent*w/100)), '>', repmat(' ', 1, w - round(percent*w/100)), ']'];
        disp(message);
    end
end
