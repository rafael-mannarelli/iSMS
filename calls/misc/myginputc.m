...
if N == 0 || ~isempty(getappdata(0,'hInvisibleAxes'))
...
%         hAx = gca;
        ...
        axSz = getpixelposition(hAx);
        if pt(1)>axSz(3) || pt(3)>axSz(4)
            exitFcn()
            return
        end
            
        % Get position inside axes
        x = [x; pt(1)];
        y = [y; pt(3)];
        ax = [ax; hAx];
...
%--------------------------------------------------------------------------
    ...
    function resetginput()
        ...
        try rmappdata(0,'hInvisibleAxes'); end % Clean up
