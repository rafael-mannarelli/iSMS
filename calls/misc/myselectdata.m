function [pointslist,xselect,yselect] = myselectdata(varargin)
...
while selectionflag
  ...
  switch params.SelectionMode
    ...
    case 'lasso'
      ...
      % mouse click?
      try but = waitforbuttonpress();
          if but~=0
              return
          end
      end
      if ~isempty(getappdata(0,'stopselectdata'))
          try figure(getappdata(0,'mainhandle')), end
          return
      end
...
function lassomotion(src,evnt) %#ok
  ...
  if strcmp(params.Identify,'on') || strcmp(params.Label,'on') % Added by SP
      [pointslist,xselect,yselect,nsel] = testpoly(xv,yv,xdata,ydata);
  end
