...
for i = 1:length(data)
    ...
    if isempty(data{i}) || size(data{i},1)<=2
        % Remove column
        x = x(:,1:end-size(data{i},2));
        continue
    end
...
elseif strcmpi(choice,'workspace')
    ...
    % Show message
    mainhandles = myguidebox(mainhandles,'Success!','Data saved to MATLAB workspace.','copydatatoworkspace');
    
elseif strcmpi(choice,'file')

    % Export to ASCII file
    [name, dir, ext] = uiputfile3(mainhandles,'export',{'*.*' 'All files'},'Export','data');
    filename = fullfile(dir,name);
    dlmwrite(filename,x,'delimiter','\t')
    
    % Show message
    mainhandles = myguidebox(mainhandles,'Success!','Data saved to file.','copydatatofile');
end
